  ...
  const modifyPDF = async () => {
    ...
    newPages.forEach((page, index) => {
      ...
      // if (pageIndex >= fromPage && pageIndex <= toPage) {
        ...
        let pageCountText=`Page ${fromPageNo ? count++ : pageIndex} / ${
    ...
    const modifiedName=`${originalname.replace('.pdf',"_modified.pdf")}`
    saveAs(modifiedPdfBlob, `${pdfFile.name}`);
    console.log("name of saved file ::", modifiedName);
